 ...
 *************************************************************************************************** 
 ...
 //9月21日，增加光电开关外部中断, PC0->光电开关1，PF4->光电开关2，PF13->光电开关3，每个开关有独立的外部中断源
 //调试PB1->热耦联锁，PF5->温度流量联锁预留外部IO口，
 //9月21日，查阅正点原子USB Host U盘实验
 //9月22日，移植USB Host U盘功能和FATFS文件系统，需要先移植SRAM内存管理。
 //9月23日，安装新的联轴器，运行稳定良好。
 //9月底和10月初，整理扫描磁铁测试平台的设计评审PPT。
 //10月19日，U盘和FATFS文件系统移植成功。
 //10月19日，开始移植RTC实时时钟。
 //10月20日~21日，升级串口模块代码，增加指令时，不需要修改复杂的解析函数，只添加新命令的结构解析函数和相关的结构体即可。
 //10月24日，调试串口解析模块的代码
 //10月25日，串口解析时间命令包添加成功，调试FATFS的时间戳命名文件成功;编写测试中创建文件，并向文件写入数据功能，创建文件成功，
 //写入文件成功，还差写入测试数据没完成。】
 //等待三轴滑轨到厂，标定测试孔径区域测试。系统软件已基本完成。
 //11月4日，调试ADC1的4个通道DMA模式成功；开始调试ADC3的3个通道DMA模式，连续+扫描。
