...
//    fattester.fatbuf = (uint8_t *)mymalloc(SRAMIN, 512);        /* 为fattester.fatbuf申请内存 */
    fattester.file = (FIL *)malloc(sizeof(FIL));      /* 为file申请内存 */
    fattester.fatbuf = (uint8_t *)malloc(512);        /* 为fattester.fatbuf申请内存 */
...
uint8_t mf_scan_files(uint8_t *path)
{
    ...
    if (res == FR_OK)
    {
        ...
        while (1)
        {
            ...
            printf("Modify time %02d:%02d:%02d\r\n", (fattester.fileinfo.ftime&0xf800)>>11,
            						 (fattester.fileinfo.ftime&0x7e0) >>5,
									 (fattester.fileinfo.ftime&0x1f)<<1);
